...
~reddigg.el~ uses [[https://github.com/dmgerman/browser-gt][browser-gt]] to run Reddit requests inside your browser.
...
- ~ht~ 2.3 or later.
- ~org~ 9.2 or later.
- [[https://github.com/dmgerman/browser-gt][browser-gt]] with its Emacs package and browser extension.
- A logged-in Reddit session in the browser.

* Setup

1. Install the required Emacs packages.
2. Install and configure ~browser-gt~.
3. Start the ~browser-gt~ Emacs client.
4. Load the ~browser-gt~ browser extension.
...
~reddigg-browser-gt-client~ selects a browser client when more than one client connects.
...
~reddigg-browser-gt-tab-wait-timeout~ sets the wait time for a new Reddit tab.
